...
export async function createSportsItem(form: FormData) {
    const res = await api('POST', '/artigosEsportivos', { data: form })
...
export async function updateSportsItem(form: FormData) {
    ...
    const res = await api('POST', `/artigosEsportivos/${id}`, { data: form })
...
export async function destroySportsItem(id: string) {
    const res = await api('DELETE', `/artigosEsportivos/${id}`)
